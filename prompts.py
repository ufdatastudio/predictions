...
class SentenceClassificationPrompt(BasePrompt):
    ...
    def few_shot(self, dataset_path: str = None, stratify_columns: list = None, seed: int = 42):
        """
        Few-shot prompting: Provides examples for each TOLSA-M property.
        Returns system identity, task, format output, and examples.
        ...
        stratify_columns : list of str, optional
            ...
            Default: ['Ground Truth'] for single-level stratification
        seed : int
        ...
        if dataset_path:
            ...
            # Multi-level stratified sampling to get representative examples
            few_shot_df = DataProcessing.multi_stratified_sample(
                train_df, 
            ...
            for idx, row in few_shot_df.iterrows():
                ...
                label_name = "TOLSA-M" if label == 1 else "non-TOLSA-M"
                
                # Optionally include dataset info if available
                dataset_info = ""
                if 'Dataset Name' in row:
                    dataset_info = f" [Dataset Name: {row['Dataset Name']}]"
