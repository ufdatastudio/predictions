#!/bin/bash
# run_ml_pipeline-in_domain-spacy_transformer.sh - Baseline with three seeds
#
# Usage:
#   chmod +x run_ml_pipeline-in_domain-spacy_transformer.sh
#   bash run_ml_pipeline-in_domain-spacy_transformer.sh
set -e
cd ../../../prediction_classification_experiments-v2
echo "Starting ML Pipeline (In-Domain Baseline) — spacy_transformer"
...
for seed in 3 7 33; do
    ...
    echo "============================================================"
    echo ""
    echo ">>> Running Baseline (Standard) — spacy_transformer"
    TOKENIZERS_PARALLELISM=false PYTORCH_DEVICE=cuda python ml-train.py \
        --dataset ../data/combined_datasets/eacl_2026_results/eacl_2026_results.csv \
        --val_size 0.2 \
        --seed $seed \
        --embedding_model spacy_transformer
done
# ============================================================
# AGGREGATE RESULTS
# ============================================================
...
echo "======================================"
echo "✓ spacy_transformer experiments completed for seeds: 3, 7, 33"
