...
class GRU_Linear(nn.Module):
    """
    GRU using nn.Linear layers for sentence classification.

    ---------------------------------------------------------------------
    DIFFERENCE FROM RNN_Linear -- this is the ONLY class that changes.
    ---------------------------------------------------------------------
    An RNN cell has one linear map (input_to_hidden) and one activation:
        h_t = tanh(Linear([x_t ; h_{t-1}]))

    A GRU cell has THREE linear maps and gated mixing:
        r_t  = sigma(Linear_reset ([x_t ; h_{t-1}]))          reset gate
        z_t  = sigma(Linear_update([x_t ; h_{t-1}]))          update gate
        h~_t = tanh (Linear_cand ([x_t ; r_t (*) h_{t-1}]))   candidate
        h_t  = (1 - z_t) (*) h_{t-1} + z_t (*) h~_t           new state

    (*) is the elementwise product. The (1 - z_t) (*) h_{t-1} term is the
    "gradient highway" -- no weight matrix and no tanh touch it, so a unit
    that learns z_t ~ 0 lets gradients pass backward through time
    untouched. That path is the whole reason a GRU carries information
    further than a vanilla RNN.

    Everything ELSE in this file is byte-for-byte the RNN pipeline, because
    the loop only relies on the shared contract:
        (x_t, hidden) -> (new_hidden, output),  hidden shape (1, H).
    ---------------------------------------------------------------------

    Output note: forward returns a probability via sigmoid, exactly like
    RNN_Linear, so it stays a drop-in with nn.BCELoss and the `> 0.5`
    threshold in evaluate_model. (If you later switch to
    BCEWithLogitsLoss, return the raw logits here and apply torch.sigmoid
    in eval before thresholding.)
    """

    def __init__(self, input_embedding_size, hidden_size, output_size):
        super(GRU_Linear, self).__init__()
        # SAME as RNN: derive D from the sample embedding tensor.
        self.input_embedding_size = input_embedding_size
        input_size = self.input_embedding_size.size()[0]
        self.hidden_size = hidden_size

        # DIFFERENT: three separate Linear layers, not one.
        # They MUST be distinct objects -- a Linear owns its weight+bias, so
        # reusing one layer for reset and update would make r_t == z_t for
        # every input and the gating would do nothing.
        self.reset_gate = nn.Linear(input_size + hidden_size, hidden_size)
        self.update_gate = nn.Linear(input_size + hidden_size, hidden_size)
        self.candidate = nn.Linear(input_size + hidden_size, hidden_size)

        # SAME as RNN: output head + sigmoid.
        self.hidden_to_output = nn.Linear(hidden_size, output_size)
        self.sigmoid = nn.Sigmoid()

        # SAME idea as RNN (Xavier), just applied to all four layers.
    ...
    def forward(self, input_tensor, hidden_tensor):
        ...
        input_tensor : torch.Tensor
            Current word embedding, shape (1, D)
        hidden_tensor : torch.Tensor
            Previous hidden state, shape (1, H)
        ...
        # SAME as RNN: concat current word with full previous state.
        i_h = torch.cat((x_t, h_prev), dim=1)                    # (1, D + H)

        # DIFFERENT: two gates. sigmoid squashes each to [0, 1].
        # r_t = how much of the past the candidate may see.
        # z_t = how much of the state this timestep rewrites.
        reset_t = torch.sigmoid(self.reset_gate(i_h))            # (1, H)
        update_t = torch.sigmoid(self.update_gate(i_h))          # (1, H)

        # DIFFERENT: candidate. Gate h_prev BEFORE concatenating -- that is
        # the reset gate's entire job. Reusing i_h here would let an ungated
        # copy of the past leak in through the concat. tanh bounds it to
        # [-1, 1] so the self-feeding state cannot blow up.
        gated = torch.cat((x_t, reset_t * h_prev), dim=1)        # (1, D + H)
        h_tilde = torch.tanh(self.candidate(gated))              # (1, H)

        # DIFFERENT: convex mixture between keeping h_prev and taking the
        # candidate. This replaces the RNN's plain `hidden_t = tanh(...)`.
        hidden_t = (1 - update_t) * h_prev + update_t * h_tilde  # (1, H)

        # SAME as RNN: project to output, squash to a probability.
        # Note we return hidden_t (the gated state), NOT h_tilde -- handing
        # the candidate forward would discard gating across timesteps and
        # silently collapse this back into a vanilla RNN.
        y_hat = self.hidden_to_output(hidden_t)                  # (1, output)
        output = self.sigmoid(y_hat)
        return hidden_t, output

    def resize_hidden(self):
        """Initialize hidden state to zeros. SAME as RNN (single tensor)."""
...
def train_step(classifier, sequence_of_embeddings, y, criterion, optimizer):
    """
    Train the model on a single sentence sequence.
...
def train_model(train_embeddings_df, classifier, n_epochs, learning_rate, optimizer_name):
    ...
    loss_history = []
    accumulation_steps = 32 # Simulate a batch size of 32
    ...
    for n_iter in range(n_epochs):
        ...
        batch_loss_sum = 0 # Tracker for accumulated loss
        ...
        if sentences_per_iteration > 0:
            ...
            # Step the scheduler based on the epoch's average loss
...
def evaluate_model(embeddings_df, original_df, classifier, dataset_name="test"):
    ...
    # GUARD (#6): the zip below aligns predictions to unique sentences BY
    # POSITION. y_preds has one entry per contiguous run of same-text rows;
    # results_df has one row per unique sentence text (first-appearance order).
    # Those match only if every sentence's word-rows are contiguous. If a
    # sentence TEXT appears in two non-adjacent blocks, run-count > unique-count,
    # the zip truncates, and every prediction after the split attaches to the
    # WRONG sentence -> silently corrupted metrics. Fail loud instead.
    # (Permanent fix for the merged file: build sentence_to_pred inside the
    # prediction loop keyed by text, so there is no position-based zip at all.)
    if len(y_preds) != len(results_df):
        raise ValueError(
            f"Prediction/sentence misalignment in '{dataset_name}': "
            f"{len(y_preds)} predictions vs {len(results_df)} unique sentences. "
            f"A sentence text likely appears in non-contiguous rows. Sort rows so "
            f"each sentence's words are grouped, or dedup upstream."
        )
    ...
    final_df['GRU'] = final_df['Base Sentence'].map(sentence_to_pred)
    final_df['GRU_prob'] = final_df['Base Sentence'].map(sentence_to_prob)
...
def compute_metrics(test_final_df, loss_history, seed, train_accuracy=None, val_accuracy=None):
    ...
    y_hat = test_final_df['GRU'].values
    y_prob = test_final_df['GRU_prob'].values  # Get continuous probabilities for AUC
...
def create_experiment_log(args, experiment_name, seed_dir, loss_history):
    ...
    log_lines = []
    log_lines.append("="*40)
    log_lines.append("GRU EXPERIMENT LOG")
    log_lines.append("="*40)
...
if __name__ == "__main__":
    print("\n" + "="*40)
    print("GRU CLASSIFIER PIPELINE")
    print("="*40)

    base_data_path = DataProcessing.load_base_data_path(script_dir)
    default_save_path = os.path.join(base_data_path, 'classification_results/')

    parser = argparse.ArgumentParser(description='Train GRU for sentence classification')
    ...
    parser.add_argument('--run_name', type=str, default='gru',
    ...
    gru_classifier = GRU_Linear(input_embedding, args.hidden_size, output_size=1)
    print(f"Model: GRU_Linear(input_size={input_embedding.size()[0]}, hidden_size={args.hidden_size}, output_size=1)")

    # ============================================================
    # 4. TRAIN MODEL
    # ============================================================
    trained_model, loss_history = train_model(
        train_embeddings_df, gru_classifier, args.n_epochs, args.learning_rate, args.optimizer
    )
    ...
    y_train_pred = train_final_df['GRU'].values
    ...
    if val_embeddings_df is not None and val_df is not None:
        ...
        y_val_pred = val_final_df['GRU'].values
    ...
    DataProcessing.save_to_file(
        test_final_df, in_domain_dir, 'gru_predictions', 'csv', include_version=False
    )
    print(f"✓ Saved predictions to: {os.path.join(in_domain_dir, 'gru_predictions.csv')}")

    # Save metrics
    DataProcessing.save_to_file(
        metrics_df, in_domain_dir, 'metrics_summary_gru', 'csv', include_version=False
    )
    print(f"✓ Saved metrics to: {os.path.join(in_domain_dir, 'metrics_summary_gru.csv')}")
